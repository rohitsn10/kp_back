...
class SubActivityUpdateViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        try:
            sub_activity_id = self.kwargs.get('sub_activity_id')  # Get the SubActivity ID from the URL
            sub_activity_name = request.data.get("name")  # Get the name value from the request data

            if not sub_activity_name:
                return Response({"status": False, "message": "name field must be provided."})

            try:
                # Fetch the specific SubActivity object
                sub_activity_obj = SubActivity.objects.get(id=sub_activity_id)
            except SubActivity.DoesNotExist:
                return Response({"status": False, "message": "Invalid SubActivity ID."})

            # Update the name field
            sub_activity_obj.name = sub_activity_name
            sub_activity_obj.save()  # Save the updated object to the database

            return Response({
                "status": True,
                "message": "SubActivity updated successfully.",
                "data": {"id": sub_activity_obj.id, "name": sub_activity_obj.name}  # Optional: Return the updated object data
            })
...
class SubSubActivityUpdateViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            sub_sub_activity_name = request.data.get("sub_sub_activity_name")

            if not sub_sub_activity_name:
                return Response({"status": False, "message": "sub_sub_activity_name must be a something."})

            try:
                # Fetch the specific SubSubActivityName object
                sub_sub_activity_name_obj = SubsubActivity.objects.get(id=sub_sub_activity_name_id)
            except SubsubActivity.DoesNotExist:
                return Response({"status": False, "message": "Invalid SubSubActivityName ID."})

            # Fetch all associated SubsubActivity objects
            sub_sub_activity_name_obj.name = sub_sub_activity_name
            sub_sub_activity_name_obj.save()
